---
name: cmm紧凑思维导图
description: 启动或推进一个cmm(compact indent mindmap)设计讨论。给定主题，创建文件，写草案，按*批注迭代。
---
...
全程遵守
  ...
  实事求是，实在，质朴，但"简单"这个词被污染了会导致省略倾向
...
格式（以下内容自身即cmm格式示范）
  ...
  试验型功能
    cmm式交流
      ...
      核心机制: AI同时维护一个活cmm文件，作为实时进度画布
        显然新话题抽一个新cmm文件，模糊有关联的就继续在当前cmm里
        cmm标题是本cmm的主题简述，4~6字，推荐4字以内，适时更新而非永远不变
        用户用自然语言说话，AI负责把query重建为cmm结构写进去
